...
public class CountCheckpointsCheck : MonoBehaviour
{
    public int levelNo;
    public int objectiveNo;

    public int checkpointsToCollect;
    ...
    void Update()
    {
        ...
        if (CheckPoints.CheckPointNum >= checkpointsToCollect && objectiveComplete == false && Ghost_Recorder.levelFinished == true)
        {
            AirCarControl.INSTANCE.objectiveBonus += 20;
            Debug.Log("COUNT CHECKPOINTS CHECK bonus = " + AirCarControl.INSTANCE.objectiveBonus);
            objectiveComplete = true;
        }
    }
