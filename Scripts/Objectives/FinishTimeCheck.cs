...
public class FinishTimeCheck : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (countdownTimer.TimerOn == false)
        {
            timer += Time.deltaTime;
        }

        if (timer <= targetFinishTime && Ghost_Recorder.levelFinished == true && objectiveComplete == false)
        {
            AirCarControl.INSTANCE.objectiveBonus += 20;
            Debug.Log("FINISH TIME CHECK bonus = " + AirCarControl.INSTANCE.objectiveBonus);
            objectiveComplete = true;
        }
    }
