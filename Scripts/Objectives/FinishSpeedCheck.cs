...
public class FinishSpeedCheck : MonoBehaviour
{
    public int levelNo;
    public int objectiveNo;

    public float targetFinishSpeed;
    private float SPEED;
    ...
    void Update()
    {
        ...
        if (Ghost_Recorder.levelFinished == true && objectiveComplete == false)
        {
            SPEED = Ghost_Recorder.speed * speedMultiplier;

            if (SPEED >= targetFinishSpeed)
            {
                AirCarControl.INSTANCE.objectiveBonus += 20;
                Debug.Log("FINISH SPEED CHECK bonus = " + AirCarControl.INSTANCE.objectiveBonus);
                objectiveComplete = true;
            }
        }
    }
